...
I stole this Kata from a GitHub repo (link below).

Write One Gherkin Feature file for each iteration.

Write Jest tests for each Scenario in each Feature file.

There is one test already created and working. Tyoe `yarn test` to run the tests manually.

Let me know when you finish each iteration and we can discuss!
